...
### Реализация
 ...
 Время работы составит $n\log n$, согласно основной теореме о рекурсии: $T(n) = 2T(n/2) + n =⇒ T(n) = n\log n$.
 Потребляемая память составит $O(n)$, поскольку необходимо завести массив для слияний. Также алгоритму нужно $O(\log n)$
